...
@interface EditVideoViewController ()<UITextFieldDelegate,UICollectionViewDelegate,UICollectionViewDataSource,JYVideSliderDelegate>
...
@property (nonatomic, strong) UICollectionView *cutCollectionView;
...
@property (nonatomic, strong) NSURL *seletedURL;
@property (nonatomic, strong) UIButton *cancleBtn;

///展示的图片
@property (nonatomic, strong) GPUImagePicture *showPicture;
...
- (void)viewDidLoad {
    ...
    if (self.image) {
        ...
        //_showPicture处理源图片
        _showPicture = [[GPUImagePicture alloc] initWithImage:self.image];
        //滤镜
        filter = [[FSKGPUImageBeautyFilter alloc] init];
        [filter forceProcessingAtSize:self.filterView.sizeInPixels];
        [_showPicture addTarget:filter];
...
//    _bgImageView.image = [[AppDelegate appDelegate].cmImageSize getImage:[[videoURL absoluteString ] stringByReplacingOccurrencesOfString:@"file://" withString:@""]];
    ...
    [self.slider getMovieFrame:self.videoModelArray.firstObject.videoModel.videoURL];
...
//    backBtn setImage:<#(nullable UIImage *)#> forState:<#(UIControlState)#>
    ...
    _cutvideoBtn = [[UIButton alloc] init];
    _cutvideoBtn.hidden = YES;
    [_cutvideoBtn setTitle:@"剪切" forState:UIControlStateNormal];
    [_cutvideoBtn.titleLabel setFont:[UIFont systemFontOfSize:15]];
    [_cutvideoBtn setTitleColor:[UIColor colorWithRed:250/256.0 green:211/256.0 blue:75/256.0 alpha:1] forState:UIControlStateSelected];
    [_cutvideoBtn setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
    _cutvideoBtn.backgroundColor = [UIColor clearColor];
    [_cutvideoBtn addTarget:self action:@selector(cutvideoBtnClicked) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:_cutvideoBtn];
    [_cutvideoBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(headerBar.mas_bottom);
        make.height.equalTo(@(35));
        make.width.equalTo(@(SCREEN_LayoutScaleBaseOnIPHEN6(35)));
    }];
    ...
    [_cutBtn setTitle:@"切割" forState:UIControlStateNormal];
    [_cutBtn.titleLabel setFont:[UIFont systemFontOfSize:15]];
    [_cutBtn setTitleColor:[UIColor colorWithRed:250/256.0 green:211/256.0 blue:75/256.0 alpha:1] forState:UIControlStateSelected];
    [_cutBtn setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
    ...
    [_musicBottomBar addSubview:_cutBtn];
    [_cutBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_musicBottomBar);
        make.left.equalTo(_stickersBtn.mas_right).offset(0);
        make.height.equalTo(@(35));
        make.width.equalTo(@(SCREEN_LayoutScaleBaseOnIPHEN6(83)));
    }];
...
//    layout2.sectionHeadersPinToVisibleBounds = YES;
    ...
    layout4.itemSize = CGSizeMake(83, 115);
    layout4.estimatedItemSize = CGSizeMake(83, 115);
    ...
    _cutCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 45, SCREEN_WIDTH, 115) collectionViewLayout:layout4];
    
    //设置背景颜色
    _cutCollectionView.backgroundColor = [UIColor clearColor];
    
    
    // 设置数据源,展示数据
    _cutCollectionView.dataSource = self;
    //设置代理,监听
    _cutCollectionView.delegate = self;
    
    // 注册cell
    [_cutCollectionView registerClass:[MusicItemCollectionViewCell class] forCellWithReuseIdentifier:@"MyCollectionCell4"];
    
    /* 设置UICollectionView的属性 */
    //设置滚动条
    _cutCollectionView.showsHorizontalScrollIndicator = NO;
    _cutCollectionView.showsVerticalScrollIndicator = NO;
    
    //设置是否需要弹簧效果
    _cutCollectionView.bounces = YES;
    
    [_musicBottomBar addSubview:_cutCollectionView];
    _cutCollectionView.hidden = YES;
...
//    [self showEditMusicBar:_musicBtn];
    ...
    }else
    {
        ...
        _stickersCollectionView.hidden = YES;
        _cutCollectionView.hidden = _slider.hidden = _cutvideoBtn.hidden = YES;
        _cutBtn.selected = NO;
    }
    if (_audioPath) {
...
-(void)cutBtnClicked
{
    ...
    }else
    {
        ...
        _stickersBtn.selected = _okBtn.selected = _musicBtn.selected = NO;
        _collectionView.hidden = YES;
        _musicCollectionView.hidden = YES;
        _collectionView.hidden = YES;
        _cutCollectionView.hidden = NO;
        _cancleBtn.hidden = NO;
        _slider.hidden = _cutvideoBtn.hidden = NO;
    }
}
#pragma mark -- 点击滤镜
...
//    [_audioPlayer pause];
    ...
    }else
    {
        ...
        _musicCollectionView.hidden = YES;
        _stickersCollectionView.hidden = _slider.hidden = _cutvideoBtn.hidden = YES;
        _cutCollectionView.hidden = YES;
        _cutBtn.selected = NO;
...
- (void)JYVideoSlider:(JYVideoSlider *)slider endTime:(CGFloat)endTime
{
    ...
    [mainPlayer pause];
    // 快进 / 快退
    CMTime time = CMTimeMakeWithSeconds(endTime, mainPlayer.currentTime.timescale);
...
{
    kWeakSelf;
    AVAsset *avasset = [AVURLAsset URLAssetWithURL:self.seletedURL options:nil];
    ...
    [HXPhotoTools exportEditVideoForAVAsset:avasset timeRange:range presetName:AVAssetExportPresetHighestQuality success:^(NSURL *videoURL) {
        ...
        [self.videoModelArray enumerateObjectsUsingBlock:^(VideoModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj.videoModel.videoURL isEqual:self.seletedURL]) {
                weakSelf.seletedURL = videoURL;
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [weakSelf.cutCollectionView reloadData];
        ...
        [HXPhotoTools exportEditVideoForAVAsset:avasset timeRange:range2 presetName:AVAssetExportPresetHighestQuality success:^(NSURL *videoURL) {
            ...
            [weakSelf.videoModelArray enumerateObjectsUsingBlock:^(VideoModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if ([obj.videoModel.videoURL isEqual:weakSelf.seletedURL]) {
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    ...
    else if (collectionView == _musicCollectionView)
    {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    // 1.从缓存池中取
    if (collectionView == _collectionView) {//贴纸
        ...
        return cell;
    }else if (collectionView == _stickersCollectionView)//滤镜
    {
        static NSString *cellID1 = @"MyCollectionCell3";
        MusicItemCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellID1 forIndexPath:indexPath];
        ...
        cell.CheckMarkImgView.hidden = !data.isSelected;
        return cell;
    }else if (collectionView == _musicCollectionView){//音乐
        ...
        static NSString *cellID3 = @"MyCollectionCell4";
        MusicItemCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellID3 forIndexPath:indexPath];
        HXPhotoModel* model = [_videoModelArray objectAtIndex:indexPath.row].videoModel;
        cell.iconImgView.image = model.previewPhoto;
        return cell;
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (collectionView == _collectionView) {//滤镜
        ...
        if (indexPath.row == 0) {
            ...
            if (self.image) {
                [_showPicture removeAllTargets];
                GPUImageFilter *fsFilter = [[FSKGPUImageBeautyFilter alloc] init];
                [_showPicture addTarget:filter];
                [filter addTarget:fsFilter];
                [fsFilter addTarget:self.filterView];
                [_showPicture processImage];
            }else{
                [movieFile addTarget:filter];
                [filter addTarget:_filterView];
    ...
    }else if (collectionView == _stickersCollectionView){//贴纸
...
//            FilterData* dataNow = [_filterAry objectAtIndex:indexPath.row];
    ...
    }else if (collectionView == _musicCollectionView){//背景音乐
        ...
        NSURL *url = self.videoModelArray[indexPath.row].videoModel.videoURL;
        if ([self.seletedURL isEqual:url]) {
            return;
        }
        ...
        [movieFile addTarget:filter];
        [filter addTarget:_filterView];
        [self.slider getMovieFrame:self.seletedURL];//消耗内存
        [mainPlayer play];
        [movieFile startProcessing];
...
{
    if (!_slider) {
        _slider = [[JYVideoSlider alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height * 0.63, self.view.frame.size.width, 50)];
